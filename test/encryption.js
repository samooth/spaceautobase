...
test('encryption - basic', async t => {
  ...
  const base = new SpaceAutobase(store, { apply, open, ackInterval: 0, ackThreshold: 0, encryptionKey: b4a.alloc(32).fill('secret') })
...
test('encryption - restart', async t => {
  ...
  const base2 = new SpaceAutobase(store2, { apply, open, ackInterval: 0, ackThreshold: 0 })
...
test('encryption - expect encryption key', async t => {
  ...
  const base = new SpaceAutobase(store, { apply, open, ackInterval: 0, ackThreshold: 0, encrypted: true })
