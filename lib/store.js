...
const [NS_SIGNER_NAMESPACE, NS_VIEW_BLOCK_KEY] = crypto.namespace('spacebase', 2)
...
class CompatExtension {
  constructor (base, core) {
    ...
    this.core = core
    this.unlinked = false
    this.extension = this.core.registerExtension('spacebase', {
...
class ViewCore {
  ...
  migrated (base, core) {
    // TODO: close old core if present, for now we just close when the spacebase is closed indirectly
...
class AutoStore {
  ...
  async _preload (name) {
    ...
    if (!this.base.opening) throw new Error('Spacebase failed to open')
