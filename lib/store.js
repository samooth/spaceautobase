...
const [NS_SIGNER_NAMESPACE, NS_VIEW_BLOCK_KEY] = crypto.namespace('spaceautobase', 2)
...
class CompatExtension {
  constructor (base, core) {
    ...
    this.core = core
    this.unlinked = false
    this.extension = this.core.registerExtension('spaceautobase', {
...
class ViewCore {
  ...
  migrated (base, core) {
    // TODO: close old core if present, for now we just close when the spaceautobase is closed indirectly
...
class AutoStore {
  ...
  async _preload (name) {
    ...
    if (!this.base.opening) throw new Error('SpaceAutobase failed to open')
  ...
  getBootstrapSystemKey () {
    return Spacecore.key(this._createManifest([], '_system', null))
  }
  ...
  async findViewByKey (key, indexers) {
    ...
    for (const v of this.byName.values()) {
      ...
      const manifestKey = Spacecore.key(manifest)
