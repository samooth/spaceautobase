...
const { partialSignature } = require('bitspacecore/lib/multisig.js')
...
module.exports = class ApplyState extends ReadyResource {
  ...
  async _open () {
    ...
    await sysCore.setUserData('spaceautobase/view', b4a.from('_system'))
    ...
    for (let i = 0; i < system.views.length; i++) {
      ...
      await v.core.setUserData('spaceautobase/view', b4a.from(v.name))
  ...
  async finalize (key) {
    ...
    await local.setUserData('spaceautobase/boot', pointer)
