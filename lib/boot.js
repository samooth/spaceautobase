...
module.exports = async function boot (corestore, key, { encrypt, encryptionKey, keyPair } = {}) {
  ...
  if (key) {
    ...
    const localKey = await bootstrap.getUserData('spaceautobase/local')
    ...
    if (key) {
      ...
      await result.bootstrap.setUserData('spaceautobase/local', result.local.key)
    }
  }
  ...
  const [encryptionKeyBuffer, pointer] = await Promise.all([
    result.local.getUserData('spaceautobase/encryption'),
    result.local.getUserData('spaceautobase/boot')
  ])
  ...
  if (!result.encryptionKey && (encryptionKey || encrypt)) {
    if (!encryptionKey) encryptionKey = (await corestore.createKeyPair('spaceautobase/encryption')).secretKey.subarray(0, 32)
    await result.bootstrap.setUserData('spaceautobase/encryption', encryptionKey) // legacy support
    await result.local.setUserData('spaceautobase/encryption', encryptionKey)
