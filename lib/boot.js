...
module.exports = async function boot (corestore, key, { encrypt, encryptionKey, keyPair } = {}) {
  ...
  if (key) {
    ...
    const localKey = await bootstrap.getUserData('spacebase/local')
    ...
    if (key) {
      ...
      await result.bootstrap.setUserData('spacebase/local', result.local.key)
    }
  }
  ...
  const [encryptionKeyBuffer, pointer] = await Promise.all([
    result.local.getUserData('spacebase/encryption'),
    result.local.getUserData('spacebase/boot')
  ])
  ...
  if (!result.encryptionKey && (encryptionKey || encrypt)) {
    if (!encryptionKey) encryptionKey = (await corestore.createKeyPair('spacebase/encryption')).secretKey.subarray(0, 32)
    await result.bootstrap.setUserData('spacebase/encryption', encryptionKey) // legacy support
    await result.local.setUserData('spacebase/encryption', encryptionKey)
