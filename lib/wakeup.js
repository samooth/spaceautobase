...
module.exports = class AutoWakeup extends ReadyResource {
  ...
  async _save () {
    ...
    await this.base.local.setUserData('spaceautobase/wakeup', slab.subarray(0, state.start))
  }

  async _load () {
    const buffer = await this.base.local.getUserData('spaceautobase/wakeup')
    if (!buffer) return
  ...
  async _add (core) {
    ...
    const viewPromise = rx.getUserData('spaceautobase/view')
