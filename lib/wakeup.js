...
module.exports = class AutoWakeup extends ReadyResource {
  ...
  async _save () {
    ...
    await this.base.local.setUserData('spacebase/wakeup', slab.subarray(0, state.start))
  }

  async _load () {
    const buffer = await this.base.local.getUserData('spacebase/wakeup')
    if (!buffer) return
  ...
  async _add (core) {
    ...
    const viewPromise = rx.getUserData('spacebase/view')
