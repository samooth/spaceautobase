...
const SpacecoreEncryption = Spacecore.DefaultEncryption

const nonce = b4a.alloc(sodium.crypto_stream_NONCEBYTES)
const hash = nonce.subarray(0, sodium.crypto_generichash_BYTES_MIN)

const [NS_HASH_KEY] = crypto.namespace('spaceautobase/encryption', 1)
...
class WriterEncryption {
  ...
  padding (context) {
    return context.manifest.version <= 1 ? SpacecoreEncryption.PADDING : WriterEncryption.PADDING
  }

  _load (context) {
    this.keys = SpacecoreEncryption.deriveKeys(this.base.encryptionKey, context.key)
  ...
  encrypt (index, block, fork, context) {
    ...
    if (this.classic) {
      return SpacecoreEncryption.encrypt(index, block, fork, this.keys.block, this.keys.blinding)
    }
  ...
  decrypt (index, block, context) {
    ...
    if (this.classic) {
      return SpacecoreEncryption.decrypt(index, block, this.keys.block)
    }
